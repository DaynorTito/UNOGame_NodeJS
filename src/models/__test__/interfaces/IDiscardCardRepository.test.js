...
describe('IDiscardCardRepository', () => {
    let repository;

    beforeEach(() => {
        const mockDiscardModel = {};
        repository = new IDiscardCardRepository({ discardModel: mockDiscardModel });
    });
    ...
    test('should throw error on bulkCreate()', async () => {
        await expect(repository.bulkCreate([])).rejects.toThrow("Method 'bulkCreate()' must be implemented.");
    });
